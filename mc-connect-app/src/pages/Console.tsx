...
export const Console = ({ logs, logEndRef }: { logs: LogEntry[], logEndRef: React.RefObject<HTMLDivElement | null> }) => {
    return (
        <motion.div
            key="console"
            initial={{ opacity: 0 }}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 font-mono text-[12px] lg:text-[14px] leading-relaxed scrollbar-terminal">
                        {logs.length === 0 ? (
                            ...
                            <div className="space-y-1.5">
                                {logs.map((log, i) => (
                                    <div key={i} className="flex gap-4 group hover:bg-slate-800/50 rounded px-2 -mx-2 transition-colors">
